...
const URL = require('../../config/config').TECHNOLOGY.NYTimesURL;
...
const crawl = async () => {
    logger.info('NYTimes Tech objects start crawling.')
    ...
    logger.info('NYTimes Tech loaded.');
    await page.waitForSelector('section#collection-technology', {timeout: 0});

    const news_list = await page.$$(
        'section#collection-technology section#stream-panel ol[aria-live="off"] li');
    logger.info('NYTimes Tech got dom.');
    ...
    logger.info('NYTimes Tech parsed all objects.')
    await News.bulkUpsertNews(allNewsResult.map(element => {
        element.platform = "NYTimes";
        return element;
    }));
    logger.info('NYTimes Tech inserted into db.')
    await page.close();
    await browser.close();
}
...
const parseNews = async (element, idx) => {
    ...
    news.categories = ['Tech', ...determineCategory(oriTitle)];
    ...
    logger.info("parsed news " + news.articleHref, {platform: "NYT Tech"});
    return news;
}
