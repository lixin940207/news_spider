...
crawl = async () => {
    ...
    await page.waitForSelector('main#site-content section#stream-panel', {timeout: 0});
    logger.info('NYTimes got dom.');

    const news = await page.$$('main#site-content section#stream-panel > div > ol > li');
    // const allEligibleLinks = (await Promise.all(zones.map(async zone => {
    //     return await zone.$$("section.story-wrapper > a[data-uri]");
    // }))).flat();
    ...
    // const newsList = [];
    for (let i = 0; i < allEligibleLinks.length; i++) {
        await parseNews(allEligibleLinks[i], i);
    }
...
parseNews = async (element, idx) => {
    let href = await element.$eval('a', node => node.getAttribute('href'));
    href = href.split('?')[0];
    if(href.endsWith('/')) href = href.substring(0, href.length - 1);
    const hrefSplit = href.split('/');
    if (hrefSplit[3] === 'interactive' || hrefSplit[4] === 'interactive' || hrefSplit[3] === 'news-event') return null;

    const news = new NewsObject();
    news.articleHref = BASE_URL + href;
    news.ranking = idx;

    objs[href].title.ori = processStr(await element.$eval('h3, h2', node => node.innerText));
    objs[href].title.cn = ENABLE_TRANSLATE? await pushToQueueAndWaitForTranslateRes(objs[href].title.ori) : "";
    objs[href].categories = determineCategory(objs[href].title.ori);

    objs[href].summary.ori = processStr(await element.$eval('p', node => node.innerText));
    objs[href].summary.cn = ENABLE_TRANSLATE? await pushToQueueAndWaitForTranslateRes(objs[href].summary.ori):"";

    objs[href].imageHref = await getImageHref(element, 'figure[aria-label="media"] img');
